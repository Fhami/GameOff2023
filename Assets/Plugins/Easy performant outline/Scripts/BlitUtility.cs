...
namespace EPOOutline
{
    public static class BlitUtility
    {
        ...
        private static Matrix4x4[] matrixBatch = new Matrix4x4[512];
        ...
        public static void Blit(OutlineParameters parameters, RenderTargetIdentifier source, RenderTargetIdentifier destination, RenderTargetIdentifier destinationDepth, Material material, CommandBuffer targetBuffer, int pass = -1, Rect? viewport = null)
        {
            ...
            if (SupportsInstancing)
            {
                ...
                {
                    var countToDraw = Mathf.Min(matrixBatch.Length, countLeft);
                    Array.Copy(matrices, itemsToDraw - countLeft, matrixBatch, 0, countToDraw);
                    countLeft -= countToDraw;
                    buffer.DrawMeshInstanced(parameters.BlitMesh, 0, material, pass, matrixBatch, countToDraw);
                }
            }
            else
        ...
        public static void Draw(OutlineParameters parameters, RenderTargetIdentifier target, RenderTargetIdentifier depth, Material material, Rect? viewport = null)
        {
            ...
            if (SupportsInstancing)
            {
                ...
                for (var index = 0; index < batchesCount; index++)
                {
                    ...
                    Array.Copy(matrices, itemsToDraw - countLeft, matrixBatch, 0, countToDraw);
                    countLeft -= countToDraw;
                    parameters.Buffer.DrawMeshInstanced(parameters.BlitMesh, 0, material, -1, matrixBatch, countToDraw);
                }
            }
            else
