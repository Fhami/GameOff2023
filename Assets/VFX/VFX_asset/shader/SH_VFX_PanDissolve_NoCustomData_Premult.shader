...
Shader "/_Kass_/SH_VFX_PanDissolve_NoCustomData_Premult"
{
	Properties
	{
		...
		_UseAlphaForDissolve("Use Alpha For Dissolve", Range( 0 , 1)) = 0
		...
		_AlphaForTransparency("Alpha For Transparency", Range( 0 , 1)) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float2 uv2_GradientShape = IN.ase_texcoord3.zw * _GradientShape_ST.xy + _GradientShape_ST.zw;
				...
				float dotResult195 = dot( tex2D( _DistortMask, ( rotator187 + ( _TimeParameters.x * _DistortPanSpeed ) ) ) , _DistortMaskChannel );
				float Distortion192 = ( dotResult195 * _DistortionPower );
				float dotResult114 = dot( tex2D( _GradientShape, ( rotator164 + uv2_GradientShape + Distortion192 ) ) , _GradientShapeChannel );
				...
				float dotResult116 = dot( tex2D( _DissolveMask, ( rotator94 + ( _TimeParameters.x * _DissolveMaskPanSpeed ) + Distortion192 ) ) , _DissolveMaskChannel );
				...
				float2 uv2_DissolveDirection = IN.ase_texcoord3.zw * _DissolveDirection_ST.xy + _DissolveDirection_ST.zw;
				...
				float lerpResult264 = lerp( 1.0 , -1.0 , _DissolveAmount);
				float lerpResult266 = lerp( -1.0 , 1.0 , IN.ase_color.a);
				float lerpResult267 = lerp( lerpResult264 , lerpResult266 , _UseAlphaForDissolve);
				float lerpResult149 = lerp( -1.0 , 1.0 , saturate( ( saturate( lerpResult144 ) + lerpResult267 ) ));
				...
				float2 uv2_ColorTexture = IN.ase_texcoord3.zw * _ColorTexture_ST.xy + _ColorTexture_ST.zw;
				float3 temp_output_39_0 = ( (tex2D( _GradientMap, temp_cast_4 )).rgb * (IN.ase_color).rgb * (tex2D( _ColorTexture, ( rotator173 + uv2_ColorTexture + Distortion192 ) )).rgb );
				...
				float dotResult115 = dot( tex2D( _Texture, ( rotator53 + Distortion192 + ( _TimeParameters.x * _MainTexturePanSpeed ) ) ) , _TextureChannel );
				...
				float temp_output_78_0 = ( saturate( dotResult115 ) * temp_output_74_0 * AdditionalAlpha247 );
				...
				float temp_output_101_0 = ( saturate( ( temp_output_78_0 + temp_output_118_0 ) ) * _AlphaBoldness );
				float lerpResult225 = lerp( temp_output_101_0 , saturate( round( temp_output_101_0 ) ) , _FlatAlpha);
				float lerpResult268 = lerp( 1.0 , IN.ase_color.a , _AlphaForTransparency);
				...
				float Alpha = saturate( ( lerpResult225 * saturate( lerpResult268 ) * saturate( lerpResult239 ) ) );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float sin133 = sin( radians( _DissolveDirectionRotation ) );
				float2 rotator133 = mul( uv_DissolveDirection - float2( 0.5,0.5 ) , float2x2( cos133 , -sin133 , sin133 , cos133 )) + float2( 0.5,0.5 );
				float2 uv2_DissolveDirection = IN.ase_texcoord2.zw * _DissolveDirection_ST.xy + _DissolveDirection_ST.zw;
				float dotResult136 = dot( tex2D( _DissolveDirection, ( rotator133 + ( uv2_DissolveDirection * _MoveWithTexture ) + Distortion192 ) ) , _DissolveDirectionChannel );
				float temp_output_146_0 = saturate( dotResult136 );
				float lerpResult144 = lerp( temp_output_146_0 , saturate( ( 1.0 - temp_output_146_0 ) ) , _DissolveDirectionInvert);
				float lerpResult264 = lerp( 1.0 , -1.0 , _DissolveAmount);
...
/*ASEBEGIN
Version=18900
210;269;1378;676;4232.202;316.2783;1.072122;True;False
Node;AmplifyShaderEditor.RangedFloatNode;182;-4831.441,1895.422;Inherit;False;Property;_DistortMaskRotation;Distort Mask Rotation;22;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.TexturePropertyNode;181;-4953.408,1559.338;Inherit;True;Property;_DistortMask;Distort Mask;20;1;[Header];Create;True;1;Distort Mask;0;0;False;0;False;None;1c5db15511bc3e1499d72c67970180f7;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.SimpleTimeNode;185;-4398.229,1930.083;Inherit;False;1;0;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.Vector2Node;183;-4372.472,2013.841;Inherit;False;Property;_DistortPanSpeed;Distort Pan Speed;23;0;Create;True;0;0;0;False;0;False;0,0;0,0.3;0;3;FLOAT2;0;FLOAT;1;FLOAT;2
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;188;-4124.229,1920.084;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT2;0,0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.SimpleAddOpNode;190;-3855.931,1895.344;Inherit;False;2;2;0;FLOAT2;0,0;False;1;FLOAT2;0,0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.Vector4Node;193;-3413.919,2109.404;Inherit;False;Property;_DistortMaskChannel;Distort Mask Channel;21;0;Create;True;0;0;0;False;0;False;0,0,0,0;1,0,0,0;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;197;-3011.625,2050;Inherit;False;Property;_DistortionPower;Distortion Power;24;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;129;-4783.844,-33.61034;Inherit;False;Property;_DissolveDirectionRotation;Dissolve Direction Rotation;17;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.TexturePropertyNode;130;-5203.246,-332.8102;Inherit;True;Property;_DissolveDirection;Dissolve Direction;15;1;[Header];Create;True;1;Dissolve Direction;0;0;False;0;False;None;bc6618725632ec24db7e39955ddf9430;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.TextureCoordinatesNode;138;-4885.846,60.98962;Inherit;False;1;-1;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;209;-4937.917,203.3546;Inherit;False;Property;_MoveWithTexture;Move With Texture;19;0;Create;True;0;0;0;False;0;False;0;1;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.GetLocalVarNode;200;-4363.635,91.47412;Inherit;False;192;Distortion;1;0;OBJECT;;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.TexturePropertyNode;63;-4562.465,409.6561;Inherit;True;Property;_DissolveMask;Dissolve Mask;8;1;[Header];Create;True;1;Dissolve Mask;0;0;False;0;False;None;b22e63ae19755cf489b2f89df1f02f5c;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.RangedFloatNode;91;-4460.683,768.1655;Inherit;False;Property;_DissolveMaskRotation;Dissolve Mask Rotation;10;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleTimeNode;152;-4017.47,782.8273;Inherit;False;1;0;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.RadiansOpNode;92;-4197.479,790.6277;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;250;-4602.178,2903.204;Inherit;False;Property;_StaticAlphaRotation;Static Alpha Rotation;6;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.Vector2Node;154;-4028.034,887.0593;Inherit;False;Property;_DissolveMaskPanSpeed;Dissolve Mask Pan Speed;12;0;Create;True;0;0;0;False;0;False;0,0;0,-0.5;0;3;FLOAT2;0;FLOAT;1;FLOAT;2
...
Node;AmplifyShaderEditor.Vector4Node;134;-3743.845,-129.6104;Inherit;False;Property;_DissolveDirectionChannel;Dissolve Direction Channel;16;0;Create;True;0;0;0;False;0;False;0,0,0,0;1,0,0,0;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.TextureCoordinatesNode;93;-4295.478,628.6274;Inherit;False;0;-1;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.TexturePropertyNode;252;-4829.928,2566.004;Inherit;True;Property;_StaticAlphaTexture;Static Alpha Texture;4;1;[Header];Create;True;1;Static Alpha;0;0;False;0;False;None;7579b8992ccf5124491219f77ea690a6;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;153;-3715.471,866.8276;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT2;0,0;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.GetLocalVarNode;202;-3674.795,1069.026;Inherit;False;192;Distortion;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.RotatorNode;94;-4015.48,654.6274;Inherit;False;3;0;FLOAT2;0,0;False;1;FLOAT2;0.5,0.5;False;2;FLOAT;0;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.SimpleAddOpNode;83;-3488.271,696.8431;Inherit;False;3;3;0;FLOAT2;0,0;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.Vector4Node;245;-3623.524,2981.163;Inherit;False;Property;_StaticAlphaChannel;Static Alpha Channel;5;0;Create;True;0;0;0;False;0;False;1,0,0,0;0,0,0,1;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.Vector4Node;66;-3214.967,680.8755;Inherit;False;Property;_DissolveMaskChannel;Dissolve Mask Channel;9;0;Create;True;0;0;0;False;0;False;0,0,0,0;0,1,0,0;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;142;-3026.386,-66.97019;Inherit;False;Property;_DissolveDirectionInvert;Dissolve Direction Invert;18;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;263;-3463.071,16.2856;Inherit;False;Property;_DissolveAmount;Dissolve Amount;14;0;Create;True;0;0;0;False;0;False;0;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.VertexColorNode;265;-3373.78,125.788;Inherit;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.TexturePropertyNode;2;-3947.794,-1194.619;Inherit;True;Property;_Texture;Texture;0;1;[Header];Create;True;1;Main Alpha;0;0;False;0;False;None;bc6618725632ec24db7e39955ddf9430;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.RangedFloatNode;55;-3720.044,-866.6188;Inherit;False;Property;_TextureRotation;Texture Rotation;2;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RadiansOpNode;54;-3476.045,-860.6188;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;261;-3190.39,320.3085;Inherit;False;Property;_UseAlphaForDissolve;Use Alpha For Dissolve;13;0;Create;True;0;0;0;False;0;False;0;1;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;266;-3158.083,176.2227;Inherit;False;3;0;FLOAT;-1;False;1;FLOAT;1;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.Vector2Node;260;-3593.063,-582.67;Inherit;False;Property;_MainTexturePanSpeed;Main Texture Pan Speed;3;0;Create;True;0;0;0;False;0;False;0,0;0,0;0;3;FLOAT2;0;FLOAT;1;FLOAT;2
Node;AmplifyShaderEditor.SimpleTimeNode;259;-3562.819,-660.428;Inherit;False;1;0;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.TextureCoordinatesNode;52;-3574.044,-1022.618;Inherit;False;0;-1;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SaturateNode;72;-2476.965,660.8755;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;264;-3141.03,15.53912;Inherit;False;3;0;FLOAT;1;False;1;FLOAT;-1;False;2;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.LerpOp;267;-2848.083,164.2227;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RotatorNode;53;-3294.045,-996.6183;Inherit;False;3;0;FLOAT2;0,0;False;1;FLOAT2;0.5,0.5;False;2;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.GetLocalVarNode;199;-3565.489,-757.7874;Inherit;False;192;Distortion;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;258;-3312.819,-664.427;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT2;0,0;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.RangedFloatNode;89;-2344.464,920.86;Inherit;False;Property;_DissolveMaskInvert;Dissolve Mask Invert;11;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;244;-3090.449,3183.544;Inherit;False;Property;_StaticAlphaInvert;Static Alpha Invert;7;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleAddOpNode;140;-3104.57,-874.7245;Inherit;False;3;3;0;FLOAT2;0,0;False;1;FLOAT;0;False;2;FLOAT2;0,0;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.Vector4Node;6;-2666.051,-559.657;Inherit;False;Property;_TextureChannel;Texture Channel;1;0;Create;True;0;0;0;False;0;False;0,0,0,0;1,0,0,0;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;104;-1245.751,390.1981;Inherit;False;Property;_CorePower;Core Power;33;1;[Header];Create;True;1;Different Center Color;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;78;-1170.609,-330.0273;Inherit;False;3;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;109;-1245.751,518.1982;Inherit;False;Property;_CoreIntensity;Core Intensity;34;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.PowerNode;117;-952.663,288.0708;Inherit;False;False;2;0;FLOAT;0;False;1;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;118;-772.2393,373.6075;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.ScreenDepthNode;234;255.2987,813.2134;Inherit;False;0;True;1;0;FLOAT4;0,0,0,0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleAddOpNode;120;-636.5406,181.324;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;102;149.7728,384.3555;Inherit;False;Property;_AlphaBoldness;Alpha Boldness;39;0;Create;True;0;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;240;347.042,1230.823;Inherit;False;Property;_DepthFadeDivide;Depth Fade Divide;42;0;Create;True;0;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;231;639.892,1026.816;Inherit;False;Property;_UseDepthFade;Use Depth Fade;41;1;[Header];Create;True;1;Depth Fade;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;262;207.193,-17.29931;Inherit;False;Property;_AlphaForTransparency;Alpha For Transparency;38;0;Create;True;0;0;0;False;0;False;1;1;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.LerpOp;268;572.6205,-134.2994;Inherit;False;3;0;FLOAT;1;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;230;652.5856,490.0969;Inherit;False;Property;_FlatAlpha;Flat Alpha;40;0;Create;True;0;0;0;False;0;False;0;1;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SaturateNode;269;862.9421,78.63147;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;158;2856.372,-435.9586;Inherit;False;Property;_ZTest;ZTest;45;0;Create;True;0;0;0;True;0;False;2;2;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.TexturePropertyNode;169;-720.3197,-1276.536;Inherit;True;Property;_ColorTexture;Color Texture;25;1;[Header];Create;True;1;Overlay Color;0;0;False;0;False;None;None;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
...
Node;AmplifyShaderEditor.RangedFloatNode;29;-71.31783,-280.1248;Inherit;False;Property;_GradientMapDisplacement;Gradient Map Displacement;31;0;Create;True;0;0;0;False;0;False;0;0.11;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;157;2346.372,-433.9586;Inherit;False;Property;_Dst;Dst;47;0;Create;True;0;0;0;True;0;False;10;10;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;51;-2974.216,-1311.579;Inherit;False;Property;_GradientShapeRotation;Gradient Shape Rotation;29;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SaturateNode;122;854.4377,-26.71929;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;156;2602.372,-433.9586;Inherit;False;Property;_ZWrite;ZWrite;44;0;Create;True;0;0;0;True;0;False;0;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.TexturePropertyNode;24;24.60729,-784.7709;Inherit;True;Property;_GradientMap;Gradient Map;30;1;[Header];Create;True;1;Gradient Map;0;0;False;0;False;None;42c68c50ab6f1c04083164fe5f373d55;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.SaturateNode;128;1636.634,-181.6373;Inherit;False;1;0;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;125;1357.696,-126.8626;Inherit;False;Property;_DifferentCoreColor;Different Core Color;35;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;43;1491.387,66.19839;Inherit;False;Property;_Brightness;Brightness;37;1;[Header];Create;True;1;Brightness and Opacity;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;95;1843.115,-476.3185;Inherit;False;Property;_Cull;Cull;43;1;[Header];Create;True;1;Rendering;0;0;True;0;False;0;2;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;159;2090.372,-433.9586;Inherit;False;Property;_Src;Src;46;0;Create;True;0;0;0;True;0;False;5;5;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;33;-385.3179,-312.1248;Inherit;False;Property;_InvertGradient;Invert Gradient;32;0;Create;True;0;0;0;False;0;False;0;1;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;170;-592.4467,-1002.408;Inherit;False;Property;_ColorRotation;Color Rotation;26;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;123;800.04,-244.9492;Inherit;False;Property;_CoreColor;Core Color;36;0;Create;True;0;0;0;False;0;False;0,0,0,0;0,0,0,0;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.Vector4Node;16;-1966.231,-1199.073;Inherit;False;Property;_GradientShapeChannel;Gradient Shape Channel;28;0;Create;True;0;0;0;False;0;False;0,0,0,0;1,0,0,0;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.TexturePropertyNode;13;-3025.837,-1592.447;Inherit;True;Property;_GradientShape;Gradient Shape;27;1;[Header];Create;True;1;Gradient Shape;0;0;False;0;False;None;e31edcb5343f39f46b9fe25b7819f528;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.SaturateNode;80;-542.7728,-381.853;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;177;2519.028,-75.02066;Float;False;True;-1;2;UnityEditor.ShaderGraph.PBRMasterGUI;0;3;/_Kass_/SH_VFX_PanDissolve_NoCustomData_Premult;2992e84f91cbeb14eab234972e07ea9d;True;Forward;0;1;Forward;8;False;False;False;False;False;False;False;False;False;False;False;False;True;0;False;-1;False;True;2;True;95;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;3;RenderPipeline=UniversalPipeline;RenderType=Transparent=RenderType;Queue=Transparent=Queue=0;True;0;0;False;True;3;1;True;159;10;True;157;3;1;True;159;10;True;157;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;True;True;True;True;0;False;-1;False;False;False;False;False;False;False;True;False;255;False;-1;255;False;-1;255;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;False;True;2;True;156;True;3;True;158;True;True;0;False;-1;0;False;-1;True;1;LightMode=UniversalForward;False;0;Hidden/InternalErrorShader;0;0;Standard;22;Surface;1;  Blend;1;Two Sided;0;Cast Shadows;0;  Use Shadow Threshold;0;Receive Shadows;0;GPU Instancing;1;LOD CrossFade;0;Built-in Fog;0;DOTS Instancing;0;Meta Pass;0;Extra Pre Pass;0;Tessellation;0;  Phong;0;  Strength;0.5,False,-1;  Type;0;  Tess;16,False,-1;  Min;10,False,-1;  Max;25,False,-1;  Edge Length;16,False,-1;  Max Displacement;25,False,-1;Vertex Position,InvertActionOnDeselection;1;0;5;False;True;False;True;False;False;;False;0
...
WireConnection;147;1;267;0
WireConnection;140;0;53;0
WireConnection;140;1;199;0
WireConnection;140;2;258;0
...
ASEEND*/
//CHKSM=27CED9D107FD7F688751669AE72A848B71B5D371